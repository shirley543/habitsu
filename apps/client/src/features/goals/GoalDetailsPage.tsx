...
import { GoalQuantifyType, type GoalResponse, type GoalStatisticsReponse } from "@habit-tracker/validation-schemas";
...
export const GoalDetailsPage = () => {
  ...
  const GOAL_STATS_DISPLAYS: Record<GoalStatsKeys, GoalStatsDisplay> = {
    yearAvg: { title: "Daily Average", units: goalData?.goalType === GoalQuantifyType.Numeric ? goalData.numericUnit : "", icon: "arrow-up-narrow-wide", labelOverride: goalData?.goalType === GoalQuantifyType.Boolean ? "N/A" : undefined },
    yearCount: { title: "Days Tracked", units: "days", icon: "timer" },
    currentStreakLen: { title: "Current Streak", units: "days", icon: "flame" },
    maxStreakLen: { title: "Longest Streak", units: "days", icon: "flame-kindling" },
  };
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {(goalData) && <>
        ...
        {(statsData) && <div className="grid grid-cols-2 grid-rows-2 gap-3">
          {
            Object.entries(statsData).map(([key, value]) => {
              const display = GOAL_STATS_DISPLAYS[key as GoalStatsKeys];
              return <div key={`statsCard_${key}`} className="bg-white rounded-xl p-2.5 shadow-sm">
                ...
                {display.labelOverride ? (<span className="text-3xl font-semibold">{display.labelOverride}</span>) :
                (<>
                  <span className="text-4xl font-semibold">{roundIfDecimal(value || 0)}</span> <span className="text-xl font-medium">{display.units}</span>
                </>)
                }
              </div>
            })
        ...
        {monthlyAvgsData && <MonthAreaChart baseColour={goalData.colour}
          valueLabel="Average"
          inputChartData={
            monthlyAvgsData.map((item) => {
              return {
                year: item.year,
                month: item.month,
                value: item.average
        ...
        {monthlyCountsData && <MonthAreaChart baseColour={goalData.colour}
          valueLabel="Completions"
          inputChartData={
            monthlyCountsData.map((item) => {
              return {
                year: item.year,
                month: item.month,
                value: item.count
